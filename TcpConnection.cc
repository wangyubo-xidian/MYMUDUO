...
// 发送数据：应用写数据快而内核发送数据慢，需要把待发送数据写入缓冲区而且设置了高水位回调
void TcpConnection::sendInLoop(const void* data, size_t len)
{
    ...
    // 已经调用过该connection的shutdown，不能再进行发送
    ...
    if (!channel_->isWriting() && outputBuffer_.readableBytes() == 0)
    {
        ...
        if (nwrote >= 0)
        {
            ...
            {
                // 数据全部发送完成就不用再给channel设置epollout事件了
    ...
    /**
     * 当前这一次write没用将数据全部发送出去，剩余的数据需要保存到缓冲区当中，然后给channel注册epollout事件,
     * poller发现tcp的发送缓冲区有空间，会通知相应的channel调用writeCallback回调方法
